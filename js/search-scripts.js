    ...
    if ( ctEvents ) {
        ctEvents.on('ct:modal:opened', ($element) => {
            if ($element && $element.id === 'search-modal') {
                ...
                const modalTaxonomies = $element.querySelector('.brasiliana-search-modal-facets-list');
                if (!modalTaxonomies) {
                    ...
                    if ( modalForm ) {
                        ...
                        fetch('/wp-admin/admin-ajax.php?action=brasiliana_add_facets_to_search_modal', {
                            method: 'POST',
